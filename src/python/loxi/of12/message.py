    ...
    def pretty_print(self, q):
        ...
        with q.group():
            with q.indent(2):
                ...
                q.breakable()
                q.text("code = ");
                value_name_map = {0: 'OFPBIC_UNKNOWN_INST', 1: 'OFPBIC_UNSUP_INST', 2: 'OFPBIC_BAD_TABLE_ID', 3: 'OFPBIC_UNSUP_METADATA', 4: 'OFPBIC_UNSUP_METADATA_MASK', 5: 'OFPBIC_BAD_EXPERIMENTER', 6: 'OFPBIC_BAD_EXPERIMENTER_TYPE', 7: 'OFPBIC_BAD_LEN', 8: 'OFPBIC_EPERM'}
                if self.code in value_name_map:
                    q.text("%s(%d)" % (value_name_map[self.code], self.code))
                else:
                    q.text("%#x" % self.code)
...
class experimenter(message):
    ...
    def __eq__(self, other):
        ...
        return False if self.subtype != other.subtype else self.data == other.data
    ...
    def pretty_print(self, q):
        ...
        with q.group():
            with q.indent(2):
                ...
                q.text(","); q.breakable()
                q.text("flags = ");
                value_name_map = {1: 'OFPFF_SEND_FLOW_REM', 2: 'OFPFF_CHECK_OVERLAP', 4: 'OFPFF_RESET_COUNTS'}
                q.text(util.pretty_flags(self.flags, value_name_map.values()))
                q.text(","); q.breakable()
                q.text("match = ");
                q.pp(self.match)
